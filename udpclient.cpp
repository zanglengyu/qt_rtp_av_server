...
void UdpClient::init_udp_socket(QString ip, int port) {
  ...
  if (!res) {
    ...
    qDebug() << "bind ip success ip = " << hostAddress << "port:" << port
             << Qt::endl;
  }
...
Q_INVOKABLE void UdpClient::send_msg(const QString &msg) {
  ...
  int send_size =
      udp_socket_->writeDatagram(byte_data, QHostAddress("224.0.0.1"), 2000);
...
void UdpClient::parse_buffer(QNetworkDatagram data) {
  qDebug() << "receive data = " << QString(data.data().toStdString().c_str())
           << Qt::endl;
